...
def al_loop(args):
    ...
    if args.task_name == 'qqp':
        # Skip TwitterPPDB OOD test if data not available (only testing KNN performance)
    ...
    while current_iteration < total_iterations + 1:
        ...
        # Track end-to-end time for this iteration
        ...
        if X_test_ood is not None:
            if args.dataset_name == 'sst-2':
                ...
                # Skip TwitterPPDB OOD test if data not available
        ...
        # Update results dict with all timing information
        ...
        print(f"\n{'Timing Summary':^80}")
        print("-" * 80)
        print(f"  Training time:        {training_time:8.2f}s")
        print(f"  Test time:            {test_time:8.2f}s")
        print(f"  Dpool inference time: {inference_time:8.2f}s")
        print(f"  Selection time:       {selection_time:8.2f}s")
        
        # Print KNN/ANN timing if available
        if 'knn_total_time' in stats:
            print(f"\n{'KNN/ANN Details':^80}")
            print("-" * 80)
            print(f"  Method:               {stats.get('knn_method', 'N/A')}")
            if 'knn_index_type' in stats:
                print(f"  Index type:           {stats['knn_index_type']}")
            if 'knn_build_method' in stats:
                print(f"  Build method:         {stats['knn_build_method']}")
            if 'knn_index_size' in stats:
                print(f"  Index size:           {stats['knn_index_size']} vectors")
            print(f"  Build time:           {stats['knn_build_time']:8.4f}s")
            print(f"  Search time:          {stats['knn_search_time']:8.4f}s")
            if 'probability_cache_enabled' in stats:
                cache_status = "enabled" if stats['probability_cache_enabled'] else "disabled"
                print(f"  Prob cache:           {cache_status}")
                if stats['probability_cache_enabled'] and 'probability_cache_time' in stats:
                    print(f"  Cache build time:     {stats['probability_cache_time']:8.4f}s")
                if stats['probability_cache_enabled'] and 'probability_cache_hit_rate' in stats:
                    print(f"  Cache hit rate:       {stats['probability_cache_size']}/{len(X_train_current_inds)} ({stats['probability_cache_hit_rate']:.1f}%)")
            print(f"  KNN total time:       {stats['knn_total_time']:8.4f}s")
            print(f"  Avg per query:        {stats['knn_search_time']/len(X_train_remaining_inds)*1000:8.2f}ms")
        
        print(f"\n  {'ITERATION TOTAL TIME:':40s} {iteration_total_time:8.2f}s")
        print("*" * 80)
        print()
...
if __name__ == '__main__':
    ...
    # Training args
    ##########################################################################
    parser.add_argument("--do_train", default=True, type=lambda x: str(x).lower() == 'true', help="If true do train")
    parser.add_argument("--do_eval", default=True, type=lambda x: str(x).lower() == 'true', help="If true do evaluation")
    parser.add_argument("--overwrite_output_dir", default=True, type=lambda x: str(x).lower() == 'true', help="If true do evaluation")
    ...
    parser.add_argument("--init", required=False,
                        ...
                        type=str,
                        help="random or alps")
    parser.add_argument("--reverse", default=False, type=lambda x: str(x).lower() == 'true', help="if True choose opposite data points")
    ##########################################################################
    # Contrastive acquisition args
    ##########################################################################
    parser.add_argument("--mean_embs", default=False, type=lambda x: str(x).lower() == 'true', help="if True use bert mean embeddings for kNN")
    parser.add_argument("--mean_out", default=False, type=lambda x: str(x).lower() == 'true', help="if True use bert mean outputs for kNN")
    parser.add_argument("--cls", default=True, type=lambda x: str(x).lower() == 'true', help="if True use cls embedding for kNN")
    # parser.add_argument("--kl_div", default=True, type=lambda x: str(x).lower() == 'true', help="if True choose KL divergence for scoring")
    parser.add_argument("--ce", default=False, type=lambda x: str(x).lower() == 'true', help="if True choose cross entropy for scoring")
    ...
    parser.add_argument("--conf_mask", default=False, type=lambda x: str(x).lower() == 'true', help="if True mask neighbours with confidence score")
    parser.add_argument("--conf_thresh", default=0., type=float, help="confidence threshold")
    parser.add_argument("--knn_lab", default=False, type=lambda x: str(x).lower() == 'true', help="if True queries are unlabeled data"
                                                                    "else labeled")
    parser.add_argument("--bert_score", default=False, type=lambda x: str(x).lower() == 'true', help="if True use bertscore similarity")
    parser.add_argument("--tfidf", default=False, type=lambda x: str(x).lower() == 'true', help="if True use tfidf scores")
    parser.add_argument("--bert_rep", default=False, type=lambda x: str(x).lower() == 'true', help="if True use bert embs (pretrained) similarity")
    ##########################################################################
    # ANN (Approximate Nearest Neighbors) optimization args
    ##########################################################################
    parser.add_argument("--use_ann", default=False, type=lambda x: str(x).lower() == 'true', 
                        help="if True use FAISS for Approximate Nearest Neighbors (ANN) search instead of exact KNN")
    parser.add_argument("--ann_nprobe", default=None, type=int,
                        help="number of clusters to search in FAISS IVF index (only used when use_ann=True and dataset is large)")
    parser.add_argument("--use_sklearn_ann", default=False, type=lambda x: str(x).lower() == 'true',
                        help="if True use sklearn NearestNeighbors with ball_tree algorithm for approximate search")
    parser.add_argument("--ann_leaf_size", default=10, type=int,
                        help="leaf_size parameter for sklearn ball_tree (smaller = faster but less accurate, only used when use_sklearn_ann=True)")
    parser.add_argument("--use_milvus", default=False, type=lambda x: str(x).lower() == 'true',
                        help="if True use Milvus Lite for vector database KNN search")
    parser.add_argument("--milvus_index_type", default="FLAT", type=str, choices=["FLAT", "IVF_FLAT", "HNSW"],
                        help="Milvus index type: FLAT (exact), IVF_FLAT (approximate), HNSW (approximate)")
    parser.add_argument("--milvus_metric_type", default="L2", type=str, choices=["L2", "IP"],
                        help="Milvus distance metric: L2 (Euclidean) or IP (Inner Product)")
    
    ##########################################################################
    # Spark KNN args
    ##########################################################################
    parser.add_argument("--use_spark", default=False, type=lambda x: str(x).lower() == 'true',
                        help="if True use Apache Spark for distributed KNN search")
    parser.add_argument("--spark_knn_method", default="exact", type=str, choices=["exact", "lsh"],
                        help="Spark KNN method: exact (brute force) or lsh (Locality Sensitive Hashing for approximate)")
    parser.add_argument("--spark_lsh_num_tables", default=5, type=int,
                        help="Number of hash tables for LSH (more tables = higher accuracy but slower)")
    parser.add_argument("--spark_lsh_bucket_length", default=2.0, type=float,
                        help="Bucket length for LSH (smaller = more buckets = higher accuracy but slower)")
    parser.add_argument("--spark_partitions", default=None, type=int,
                        help="Number of Spark partitions (default: use Spark's default partitioning)")
    parser.add_argument("--milvus_nlist", default=100, type=int,
                        help="number of clusters for Milvus IVF_FLAT index")
    parser.add_argument("--milvus_nprobe", default=10, type=int,
                        help="number of clusters to search in Milvus IVF_FLAT index")
    parser.add_argument("--milvus_ef", default=64, type=int,
                        help="ef parameter for Milvus HNSW index (larger = more accurate but slower)")
    parser.add_argument("--use_incremental_index", default=True, type=lambda x: str(x).lower() == 'true',
                        help="if True use incremental index updates across AL iterations (saves time by reusing previous index)")
    parser.add_argument("--cache_probabilities", default=True, type=lambda x: str(x).lower() == 'true',
                        help="if True cache softmax/log_softmax probabilities to avoid recomputation (Algorithm 1 line 3 optimization)")
    ##########################################################################
    # Server args
    ##########################################################################
