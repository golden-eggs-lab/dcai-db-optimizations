...
def train_transformer_model(args, X_inds, X_val_inds=None,
    ...
    while (val_acc_current < val_acc_previous - 0.5 and times_trained < 2) or (times_trained == 0):
        ...
        # Try to load from local cache first, then from HuggingFace
        local_model_path = os.path.join(args.cache_dir, f"{args.model_name_or_path}-local")
        ...
        try:
            model = AutoModelForSequenceClassification.from_pretrained(
                ...
                config=config,
                cache_dir=args.cache_dir if model_path == args.model_name_or_path else None,
            )
        except (OSError, EnvironmentError) as e:
            logger.warning(f"Failed to load model from {model_path}: {e}")
            # If loading from HuggingFace fails, try without cache_dir
            if model_path == args.model_name_or_path:
                logger.info("Trying to load without cache_dir...")
                model = AutoModelForSequenceClassification.from_pretrained(
